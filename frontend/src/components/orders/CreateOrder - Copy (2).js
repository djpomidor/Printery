...
const CreateOrder = props => {
  ...
  const addOrder = async (props) => {
    const { number, nameOfOrder, owner, typeOfOrder, circulation, binding, width, height, created, due_date, delivery_date} = props;
    try {
  ...
  const onSubmit = async values => {
    ...
    //   e.preventDefault();
    //   e.stopPropagation();
    //   console.log("form.checkValidity() === false", e)
    // }
    // else {
    // e.preventDefault();
